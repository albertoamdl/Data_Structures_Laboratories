...
# code given from lab assignment
...
def count_anagrams(word, english_words, prefix=""):  #this method does not work and was supoost to permute word and increment count of words
    global count
    count = 0
    if len(word) <= 1:
        elem = prefix + word
...
def RB_read_file():  #method to read my file
    word_file = open("words.txt", "r")
...
def main():
    user_option = input("What type of tree would you like to use? Type 'a' for AVL or 'b' for Red-Black tree or 'c' for Btree ")
    if user_option is "a":
        avl = AVL_read_file()  # Tree
        avl.print_tree(avl.root)  # printing, not supposed to return anything
        print(count_anagrams("spot", avl, prefix=""))

    elif user_option is "b":
        r = RB_read_file()
        r.print_tree(r.root)

    elif user_option is "c":
        b = Btree_read_file()
        b.print(b.root)

        print("choose a word to find anagrams for: ")
        user_input = input()
        print_anagrams(user_input, b)
        print("total number of anagrams: ")
        count_words = count_anagrams(user_input, b)
        if count_words == 0:
